...
stateDiagram-v2
    ...
    PENDING --> IN_PROGRESS: system/dequeue
...
<TabItem value="config" label="config/default.json">

```json
{
  "jobnik": {
    "baseUrl": "https://jobnik-manager.example.com",
    "worker": {
      "concurrency": 5,
      "backoffOptions": {
        "initialBaseRetryDelayMs": 1000,
...
Jobnik provides comprehensive testing capabilities at multiple levels. For production-grade E2E test examples, see the [Jobnik E2E Test Suite](https://github.com/MapColonies/jobnik-e2e).
...
<Tabs>
<TabItem value="test" label="tests/image-resize.handler.spec.ts">
...
<Tabs>
<TabItem value="simple" label="tests/simple-workflow.spec.ts">
...
<TabItem value="multi-stage" label="tests/multi-stage-workflow.spec.ts">
...
describe('Multi-Stage Workflow', () => {
  ...
  it('should create job with multiple stages', async () => {
    const producer = sdk.getProducer();

    // Create job
    const job = await producer.createJob({
      name: 'data-pipeline',
      data: { pipelineId: 'pipeline-001', source: 's3://input' },
      userMetadata: { owner: 'data-team' },
      priority: 'MEDIUM',
    });
    ...
    const validationStage = await producer.createStage(job.id, {
      type: 'validate-data',
      data: { schema: 'v1', strictMode: true },
      userMetadata: { validatedCount: 0 },
    });

    // Create tasks for validation
    const validationTasks = await Promise.all([
      producer.createTask(validationStage.id, {
        data: { file: 'data1.json' },
        userMetadata: { index: 0 },
      }),
      producer.createTask(validationStage.id, {
        data: { file: 'data2.json' },
        userMetadata: { index: 1 },
      }),
    ]);
    ...
    const processingStage = await producer.createStage(job.id, {
      type: 'process-data',
      data: { algorithm: 'v2', batchSize: 100 },
      userMetadata: { processedCount: 0 },
    });

    // Create tasks for processing
    const processingTasks = await Promise.all([
      producer.createTask(processingStage.id, {
        data: { file: 'data1.json', outputPath: '/output/data1.parquet' },
        userMetadata: { index: 0 },
      }),
      producer.createTask(processingStage.id, {
        data: { file: 'data2.json', outputPath: '/output/data2.parquet' },
        userMetadata: { index: 1 },
      }),
    ]);
    ...
    expect(job.id).toBeDefined();
    expect(validationStage.jobId).toBe(job.id);
    expect(processingStage.jobId).toBe(job.id);
  });
});
```

</TabItem>
<TabItem value="priority" label="tests/priority-queue.spec.ts">
...
describe('Priority Queue Management', () => {
  ...
  it('should respect job priority when dequeuing tasks', async () => {
    const producer = sdk.getProducer();

    // Create LOW priority job
    const lowPriorityJob = await producer.createJob({
      name: 'background-job',
      data: { type: 'cleanup' },
      priority: 'LOW',
    });

    const lowPriorityStage = await producer.createStage(lowPriorityJob.id, {
      type: 'cleanup-task',
      data: { target: 'temp-files' },
    });

    await producer.createTask(lowPriorityStage.id, {
      data: { file: 'temp1.txt' },
    });

    // Create HIGH priority job
    const highPriorityJob = await producer.createJob({
      name: 'urgent-job',
      data: { type: 'critical-update' },
      priority: 'HIGH',
    });

    const highPriorityStage = await producer.createStage(highPriorityJob.id, {
      type: 'urgent-task',
      data: { urgency: 'critical' },
    });

    await producer.createTask(highPriorityStage.id, {
      data: { action: 'update-config' },
    });

    // Both jobs created, priority determines dequeue order
...
See the **[Jobnik E2E Test Suite](https://github.com/MapColonies/jobnik-e2e)** repository which provides:
...
<TabItem value="compose" label="docker-compose.yaml">
...
services:
  worker:
    build: .
    environment:
      - JOBNIK_BASE_URL=http://jobnik-manager:3000
      - WORKER_CONCURRENCY=5
      - LOG_LEVEL=info
    depends_on:
      - jobnik-manager
