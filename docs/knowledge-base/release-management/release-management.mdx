...
The **Libot Version** is an alias to a specific `helm-charts` release version (`helm-charts` repo is our "one source of truth" 
structure of helm and is explained below in [Helming Strategy](#helming-strategy)). This means that it represents the entirety of all 
service releases for the production deployment and it's structure.  
...
open a PR in `helm-charts` that updates the version of the service.
...
3. On push to `master`:
    ...
    `release-please` should also update the docker and helm versions of the services in:
        - `artifacts.json` file inside the domain that represent the information of all services and versions of the domain
        - `libot-artifacts.json` file in the root directory that represent the information of all services and versions of **all** domains
            :::info
            `libot-artifacts.json` is a new file presented in `helm-charts` in order to group in one file all the information of the domains 
            and the services related to `helm-charts` version. This file will be updated automatically on every change of `artifacts.json` 
            files present inside the domains folders
            :::

    - Package all the charts and push them to helm registry.  
    For example, let's assume a new release `3.0.1` where the only change was in raster ingestion. 
    In this case we will push to helm registry the `raster-ingestion` chart with the name `rater-ingestion-3.0.1`. 

**End State**:

* New `helm-charts` version 
...
0. On new release, there will be a PR on stage branch in `helm-values` repo that updates the `helm-charts` version in `helm-charts.json` file
