...
2. On merging to master `release-please` takes action and opens a release PR with these changes:
    ...
    :::info
    The release version will be based on [SemVer](https://semver.org/) (according to the PR title that will abide [commitlint](https://github.com/conventional-changelog/commitlint))
    :::
...
Currently, the charts in the defferent networks are different from the charts in Azure because they can be modified. 
...
1. When a service has a new release and a helm folder exists, meaning that it's chart in helm-charts needs to be updated, open a PR in `helm-charts` that updates the version of the service.
...
3. On push to `master`:
    ...
    For example, let's assume a new release `3.0.1` where the only change was in raster ingestion. In this case we will push to helm registry the `raster-ingestion` chart with the name `rater-ingestion-3.0.1`. 
...
1. The content of the new PR will be auto deployed to the `integration` namespace
...
3. Once the stage branch is updated, a new PR will be automatically created in "prod" branch to update the `helm-charts` version in the file
