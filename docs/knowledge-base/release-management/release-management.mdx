...
3. On push to `master`, there will be a new PR using `release-please`. The PR will be the updated chart version according to the commitlint

4. Merging the PR of `release-please`, will create a new release and attach to the release the artifacts.json

5. On new release, there will be a github workflow that will push the release to helm registry

6. After that, a new PR will be triggered to helm-values (starting the new section of [Values Flow](#values-flow))
...
0. On new release of `helm-charts`, there will be a PR on stage branch in `helm-values` repo that updates the `helm-charts` version in `helm-charts.json` file
