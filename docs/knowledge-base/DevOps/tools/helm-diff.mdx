...
The `helm diff` is a Helm plugin that shows differences between a Helm chart's current state
...
* Create a directory `helm-dff` under the `HELM_PLUGINS` path.
* From the extracted `tgz` file, copy the `bin` directory and the `plugin.yaml` into the `HELM_PLUGINS`/`helm-dff` directory.

Now you have `helm-diff` plugin installed on your Windows workstation.

## How to use it

Assuming that your `chart` name is `foo` and your `vaulues` file name is `dev-values.yaml`
run the following from the `Chart` directory you wish to upgrade.  
`helm diff upgrade foo --values dev-values.yaml .`  
If there's a `diff` the output will show all the diffs between the `k8s` objects which will be modified/create/deleted.
