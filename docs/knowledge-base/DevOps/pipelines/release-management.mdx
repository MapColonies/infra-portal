...
![Helming Strategy](./img/release-management/helm-strategy.png)
...
"helm-values" repo will contain **only the values based on the relevant chart** and the "Libot Version" it based on.
The values files that represent namespaces of openshift should not be part of the structure. They should be in a different repo.
That's way we earn:  

1. **Avoiding "trash" commits**  
Imagine if we keep the values with the chart, Now if we want to change the log level from info to debug, we will have to commit the change in the repo itself and this shouldn't be. 
...
![Chart Flow](./img/release-management/chart-flow.png)
...
3. On "master" push:
    ...
    The release-please should also update the docker and helm versions of the service in:
...
![Values Flow](./img/release-management/values-flow.png)

This flow suppose to handle the update of the namespaces values (integration and prod) and the deployment in openshift.

0. On new release, there will be a PR on stage branch in `helm-values` repo that updates the "Libot Version"
...
3. Once the stage branch is updated, a new PR will be automatically created in "prod" branch to update the "Libot Version"
...
![Bundling Strategy](./img/release-management/bundler-strategy.png)
