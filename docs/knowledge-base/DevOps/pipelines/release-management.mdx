...
The release strategy follows semantic versioning ([SemVer](https://semver.org/)) for individual services and "Libot version" for managing deployed Libot stack on networks.

## Libot Version Tag

- The **Libot Version** is assigned based on the `helm-charts` release version.
- This version represents the bundle of all service releases for the annual deployment to production.

## Service Branching Strategy

Each service must maintain at least two main branches to track the latest stable release and major version updates:
...
2. **`MAJOR.x.x`** (e.g., `1.x.x`, `2.x.x`):
   ...
   - A new branch is created for every major version.
   - Example:
     - If a service's latest release version is `2.3.0` and a new feature is added, a pull request (PR) will be created for both:
       - `master` (for stable updates)
       - `2.x.x` (to track the new version).
     - Once merged, both branches will represent the `2.4.0` release, while the `1.x.x` branch remains unchanged.
...
2. On merging to master: 
    - An automate workflow is activated to merge the PR also to the MAJOR branch
...
**End State:**
    ...
    - master and relevant MAJOR branch point to the new release
