...
{/* Replace this once docusaurus add support to mermaid v11 with architechture diagram */}
![config parts relation diagram](./img/config-relation-diagram.png)

## Motivation
You want to use the MapColonies™ Config Management system in your service to manage all the configurations in one place.
It will help you to have a single source of truth for all the configurations and make it easier to manage them.
It will also help you as it validates the configurations against the JSON Schema, so you can be sure that the configurations are correct.
Another reason is that because of the schema, all your configuration be typed, so you can use it in your code without any issues.
...
2. Create your schema content. Use the official JSON Schema docs, [The tips page](./json-schema-tips.mdx) and check other schemas in the repo for reference.
